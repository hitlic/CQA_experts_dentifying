...
def build_net():
    ...
    #添加节点
    for item in data:
        #if item[1] not in nodeid2num.keys() and item[1] != '0':
        if item[1] not in G and item[1] != '0':
            G.add_node(item[1], index =-1)
            #G.add_node(str(k), index =-1)
            #nodeid2num[item[1]] = str(k)
            #k += 1
    print('图中节点的个数:', G.number_of_nodes())#图中节点的个数:6218(现)
    '''
    #添加边
    for i in data:
        for j in data:
...
def loaddata(G):
    ...
    datas = pd.read_csv("./data/author_6220_label_40.csv", header=None, encoding='utf-8')
    labels = [-1 for i in range(len(G))]
    features = [0 for i in range(len(G))]
    for i in range(len(datas)):
        #print(datas[0][i])
        if datas[0][i] in G:
            features[G.node[datas[0][i]]['index']] =[float(datas[17][i]),#float(datas[17][i]),
