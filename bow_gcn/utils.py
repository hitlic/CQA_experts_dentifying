...
def preprocess_features(features):
    ...
    rowsum = np.array(features.sum(0))#1#【axis=0按列相加；axis=1按行相加】
    ...
    features =  features.dot(r_mat_inv)#r_mat_inv.dot(features)#【r_mat_inv点乘features】
    """
...
def construct_feed_dict(features, support, labels, labels_mask, placeholders):
    ...
    feed_dict.update({placeholders['features']: features})  # ?自动把元组转成稀疏向量值了？
    # feed_dict.update({placeholders['features']: tf.SparseTensorValue(*features)})  # 与上一行等价
    feed_dict.update({placeholders['support'][i]: support[i] for i in range(len(support))})  # ?自动把元组转成稀疏强量了？
    feed_dict.update({placeholders['num_features_nonzero']: features[1].shape})#features[1].shape})
    return feed_dict
